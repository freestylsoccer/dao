...
contract OlympusTokenMigrator is OlympusAccessControlled {
    ...
    IERC20DAO public immutable oldOHM;
    ...
    function defund(address reserve) external onlyGovernor {
        ...
        IERC20DAO(reserve).safeTransfer(address(newTreasury), IERC20DAO(reserve).balanceOf(address(this)));
    ...
    function migrateLP(
        ...
        uint256 oldLPAmount = IERC20DAO(pair).balanceOf(address(oldTreasury));
        ...
        IERC20DAO(pair).approve(address(router), oldLPAmount);
        ...
        IERC20DAO(token).approve(address(router), amountA);
    ...
    function withdrawToken(
        ...
        IERC20DAO tokenContract = IERC20DAO(tokenAddress);
    ...
    function _migrateToken(address token, bool deposit) internal {
        uint256 balance = IERC20DAO(token).balanceOf(address(oldTreasury));
        ...
        if (deposit) {
            IERC20DAO(token).safeApprove(address(newTreasury), balance);
            newTreasury.deposit(balance, token, tokenValue);
        } else {
            IERC20DAO(token).safeTransfer(address(newTreasury), balance);
        }
    }
}
